...
- [x] 🤖 Full gather → craft → prune → build loop wired in Sunhollow greybox: all four verbs advance the quest through one consistent `CompletesObjective` pattern. Gather (`AGatherNode` → `gather_sunmoss`), craft (`URecipeRow.CompletesObjective` → `craft_blade`), prune (`ABloomActor` → `prune_bloom`), build (`ALongNoonGateBuild` → `build_gate`). Objective ordering/skip logic is automation-covered (`TheLongNoon.Systems.QuestFlow`); a seeded in-level quest that lists these objectives in order is the remaining piece.
