...
- [~] 🤖 The R1→R2 gate-build is buildable: `ALongNoonGateBuild` (gate `gate_overgrowth`) marks the region gate complete via the building component so R2 becomes enterable (region gating is automation-covered, `TheLongNoon.Systems.RegionGate`). Remaining: actually load/stream a stub R2 level on completion.
