...
- [x] 🤖 Functional HUD built entirely in C++ (`ULongNoonHUDWidget`, no WBP needed): Stamina/Focus meters, interact prompt, lore toast, persistent objective tracker, and NPC dialogue line. Plus a keyboard-driven pause menu (`ULongNoonPauseWidget`) and title screen (`ULongNoonMainMenuWidget` + `ALongNoonMenuGameMode`, the game boots to it). All screenshot-verified. A WBP restyle for richer cozy art is an optional later 🤝 pass.

### Phase 4 — Sunhollow vertical slice (Region 1 fully playable)
- [x] 🤖 Full gather → craft → prune → build loop wired in Sunhollow and player-reachable end to end. All four verbs advance a `ALongNoonQuestBoard`-seeded quest via one `CompletesObjective` pattern: gather (`AGatherNode` → `gather_sunmoss`), **craft (a `ALongNoonCraftingStation` bench opens `ULongNoonCraftingMenuWidget`; crafting warm broth from gathered sunmoss → `craft_broth`)**, prune (`ABloomActor` → `prune_bloom`, behavior-verified: F at spawn reclaims + completes it), build (`ALongNoonGateBuild` → `build_gate`, opens the next region). HUD shows a live objective tracker; finishing all four fires `OnActiveQuestCompleted`. The opening is tuned so the first prune lands at spawn and the bench/nodes are a step away. Logic automation-covered (`TheLongNoon.Systems.QuestFlow`). Crafting is the formerly-missing leg — `CraftById` was wired but had no in-game trigger until the station+menu.
...
- [x] 🤖 The R1→R2 gate-build is buildable and opens a stub R2: `ALongNoonGateBuild` (gate `gate_overgrowth`) marks the region gate complete via the building component (region gating automation-covered, `TheLongNoon.Systems.RegionGate`) and, with its `TargetLevel` set, opens `L_Overgrowth_Greybox` (Region 2 stub: ground, proven lighting, R2 gather nodes `mat_swallowedwood`, a tier-2 Settle Bloom, a lore fragment). Both levels runtime-verified to boot clean in ~0.1s (`Tools/build_overgrowth.py`). The chain now extends to Region 3: the Overgrowth gate (`gate_stillworks`) opens `L_Stillworks_Greybox` (`Tools/build_stillworks.py`), each region carrying its own quest board + objective hooks (R2: gather_swallowedwood/settle_overgrowth/build_gate_2; R3: gather_stillsteel/still_stillworks/build_gate_3), all runtime-verified to seed their quest on boot. Playable chain: Sunhollow → Overgrowth → Stillworks. Packaging note: pass all three maps to `-map=` when cooking.
...
- [x] 🤖 All five region greyboxes rebuilt with the full quest-tracked pattern and chained end to end: **Sunhollow → Overgrowth → Stillworks → Hush → First Garden**. Each (per-region builder `Tools/build_<region>.py`, all runnable in one editor session via `Tools/build_chain.py`) has ground, proven golden-hour sun, region-signature gather nodes + a tier-appropriate Bloom (both with `CompletesObjective` hooks), a `ALongNoonQuestBoard` seeding the region quest, and an `ALongNoonGateBuild` whose `TargetLevel` opens the next region. All runtime-verified to boot clean (~0.1s) and seed their quest. Per-region light-dial/palette is a deferred art pass.
- [ ] 🤖 Region gating wired end to end (tool tier + traversal + gate-build) per `progression-and-gating.md`.
- [~] 🤖 NPCs placed with data-driven dialogue: Tinker + Keeper (Sunhollow), Gardener (First Garden, `[NPC] npc_gardener LoadFromData=ok lines=6`). The Rememberer in R4 is wired as the deep-lore unlock: reading `frag_remembrancer_archive` (a `ALongNoonLoreFragment` with `bGrantsRememberer`) calls `ULongNoonGameInstance::SetRemembererFound()`, which is what gates the Third Way ending (previously the flag was never set, so the secret ending was unreachable — now fixed). Remaining: place the Forager/One-Who-Counts NPCs + per-NPC story beats.
