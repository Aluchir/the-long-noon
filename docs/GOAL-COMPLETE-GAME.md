...
- [x] 🤖 Full gather → craft → prune → build loop wired in Sunhollow greybox and player-visible end to end: a `ALongNoonQuestBoard` seeds the ordered quest on BeginPlay (runtime-verified: `[Quest] Quest started with 4 objectives; active=gather_sunmoss`), all four verbs advance it through one consistent `CompletesObjective` pattern — gather (`AGatherNode` → `gather_sunmoss`), craft (`URecipeRow.CompletesObjective` → `craft_blade`), prune (`ABloomActor` → `prune_bloom`), build (`ALongNoonGateBuild` → `build_gate`) — the HUD shows a live objective tracker, and finishing all four fires `OnActiveQuestCompleted` ("The way is open."). Logic automation-covered (`TheLongNoon.Systems.QuestFlow`, drives a live subsystem through the whole quest).
